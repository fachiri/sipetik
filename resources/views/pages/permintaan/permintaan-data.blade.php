		...
		@if ($categories && auth()->user()->role == 'ADMIN')
			<div class="mb-4 flex flex-col space-y-2 text-center md:flex-row md:space-x-3 md:space-y-0">
				@foreach ($categories as $category)
					<a href="?kategori={{ $category->name }}" class="tab-btn tab-btn-permintaan {{ request('kategori') == $category->name ? 'active' : '' }} rounded-lg border-2 border-[#002979] px-3 py-2 font-bold text-[#002979]">{{ $category->name }}</a>
				@endforeach
			</div>
		@endif
					...
					@foreach ($reports as $report)
						<tr>
							...
							<td class="whitespace-nowrap">
								...
								@endphp
								<small class="{{ $bgColor }} {{ $textColor }} rounded px-2 py-1 font-semibold">
									{{ $report->prioritas }}
								</small>
							</td>
							<td class="whitespace-nowrap">
							...
							<td class="row-action--icon flex space-x-2 whitespace-nowrap">
								...
								<a role="button" href="{{ route('permintaan.detail', $report->report_id) }}" class="relative w-6">
